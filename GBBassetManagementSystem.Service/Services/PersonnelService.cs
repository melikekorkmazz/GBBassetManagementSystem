    ...
    public async Task UpdateAsync(Personnel personnel)
    {
        ...
        if (existingPersonnel is null)
        {
            throw new KeyNotFoundException(
                "Personnel was not found.");
        }

        existingPersonnel.FirstName =
            personnel.FirstName;

        existingPersonnel.LastName =
            personnel.LastName;

        existingPersonnel.RegistrationNumber =
            personnel.RegistrationNumber;

        existingPersonnel.NationalIdentityNumber =
            personnel.NationalIdentityNumber;

        existingPersonnel.Email =
            personnel.Email;

        existingPersonnel.PhoneNumber =
            personnel.PhoneNumber;

        existingPersonnel.DepartmentId =
